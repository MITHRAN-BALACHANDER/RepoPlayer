...
Usage:
    python reposcan.py record <owner/repo>   Record a cassette from real APIs
    python reposcan.py show   <owner/repo>   Display a saved report (offline)
...
def _parse_target(raw: str) -> tuple[str, str]:
    if "/" not in raw:
        ...
        print("Example: python reposcan.py record fastapi/fastapi")
        sys.exit(1)
    return raw.split("/", 1)
...
def cmd_show(target: str) -> None:
    ...
    try:
        ...
        print(f"     Record it first:  python reposcan.py record {owner}/{repo}")
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or sys.argv[1] not in COMMANDS:
        ...
        print()
        print("Usage:")
        print("  python reposcan.py record <owner/repo>   # record from real APIs")
        print("  python reposcan.py show   <owner/repo>   # display saved report")
        print()
        print("Examples:")
        print("  python reposcan.py record fastapi/fastapi")
        print("  python reposcan.py show   fastapi/fastapi")
